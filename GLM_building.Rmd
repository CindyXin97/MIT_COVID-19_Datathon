...
```{r}
statenames <- read_csv('Data/state_abbreviations.csv') %>% select(State,Code) %>% rename(STATE=Code)
...
```{r}
#read_csv('Data/covid_prison_releases.csv')
...
```{r}
county_pop <- read_csv('Data/county_demographics_df.csv') %>% rename(COUNTYFIPS = fips)
county_pop
