...
nyt_with_prisons <- merge(nytdf, prison_counties_frac_released, all.x=TRUE) %>% group_by(COUNTYFIPS) %>%
  ...
  select(COUNTYFIPS,date,cases,deaths,prison_county,frac_released,diff_cases,frac_change_cases,prison_pop,prison_capacity,prison_fullness) %>%
  unique()
...
```{r}
write.csv(prison_pop_df,'Data/full_population_and_prison_data')
```
...
county_pops <- prison_pop_df %>% select(COUNTYFIPS,tot_pop) %>% drop_na()
ggplot(county_pops) + geom_histogram(aes(tot_pop),bins=100)+ xlim(0,100000)

county_pops %>% filter(tot_pop<100) 
```


```{r}
modelx <- glm(cases_per_capita ~ tot_pop + prison_county + population_density, data = prison_pop_df)
summary(modelx)

modely <- glm(cases_per_capita ~  prison_pop + tot_pop + cases_per_capita_7_days_ago + population_density, data = prison_pop_df)
summary(modely)

modelz <- glm(deaths_per_capita ~ tot_pop + prison_county + population_density, data = prison_pop_df)
summary(modelz)

#R2 values
with(summary(modelx), 1 - deviance/null.deviance)
with(summary(modely), 1 - deviance/null.deviance)
with(summary(modelz), 1 - deviance/null.deviance)
```


target variable as deaths: deaths prevented, numbers of cases prevented (large), maybe not even week to week --> cumulative?